...
describe("Linux CLI package boundary", () => {
  ...
  it("does not reintroduce the withdrawn Office rendering payload requirement", () => {
    const builder = readFileSync(builderPath, "utf8");
    const linuxWorkflow = readFileSync(linuxWorkflowPath, "utf8");

    expect(builder).not.toContain("verify_office_rendering_bundle");
    expect(builder).not.toContain("verify_office_skill_payload");
    expect(linuxWorkflow).not.toContain("MEMMY_LINUX_CLI_ALLOW_MISSING_OFFICE_PAYLOAD");
  });
