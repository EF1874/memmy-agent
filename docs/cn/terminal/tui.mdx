...
- `/history-dag` 和 `/dream*` 是否出现取决于对应功能是否启用。
...
| `/goal clear` | 无 | 清除目标 |
| `/dream` | 无 | 执行记忆整理 |
| `/dream-log` | 无 | 查看记忆整理记录 |
| `/dream-log <sha>` | Dream 版本 SHA | 查看指定记忆整理版本的变更 |
| `/dream-restore` | 无 | 列出可恢复的记忆整理版本 |
| `/dream-restore <sha>` | Dream 版本 SHA | 恢复到指定记忆整理版本 |
| `/pairing` | 无 | 查看或管理渠道配对 |
...
`/history-dag`、`/dream*` 等命令是否可用，取决于对应功能是否在配置中启用。
